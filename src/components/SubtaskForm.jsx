...
const SubtaskForm = ({ onSubmit, onClose, editingSubtask }) => {
  const initialTime = editingSubtask?.estimatedTime || "1hr";
  const isInitialCustom = !PRESET_TIMES.includes(initialTime);

  const [formData, setFormData] = useState({
    title: editingSubtask?.title || "",
    complexity: editingSubtask?.complexity || "Medium",
      ...
      <div className="bg-zinc-900 border border-zinc-800 text-white p-6 rounded-2xl shadow-2xl space-y-5 w-full max-w-md">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            <span>📋</span> {editingSubtask ? "Edit Subtask" : "New Subtask"}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1">Subtask Title</label>
            <input
              type="text"
              name="title"
